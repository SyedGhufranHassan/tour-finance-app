...
export function AuthGate({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data, error }) => {
      if (error) {
        setMessage("Supabase connection failed. Restart the dev server and verify the Project URL and publishable key.");
      } else {
        setAuthenticated(Boolean(data.session));
      }
      setSessionReady(true);
    }).catch(() => {
      setMessage("Supabase is unreachable. Check your internet connection and Supabase project URL.");
      setSessionReady(true);
    });
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      if (result.error) {
        ...
        setMessage(result.error.message.includes("Failed to fetch")
          ? "Could not reach Supabase. Restart Vite and check your internet connection."
      ...
      setMessageTone("error");
      setMessage("Could not reach Supabase. Restart Vite and check your internet connection.");
    }
    setBusy(false);
  }
